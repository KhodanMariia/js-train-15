...
function calculateHypotenuse(a, b) {
  ...
  const lengthC = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2));
...
function roundObjectValues(obj) {
  ...
  const newKeys = keys.map(([key, value]) => {
  ...
  const newObj = Object.fromEntries(newKeys);
...
function compareAndRound(num1, num2) {
  ...
  const roundNumber = Math.round(maxNumber);
...
function estimateInvestment(principal, interestRate, years) {
  ...
  let totalAmount = principal * Math.pow(1 + interestRate, years);
